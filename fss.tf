...
resource "oci_file_storage_file_system" "FileStorage" {
  ...
  display_name        = count.index < "9" ? "${var.fss_display_name_base}${var.label_zs[0]}${count.index + 1}" : "${var.fss_display_name_base}${var.label_zs[1]}${count.index + 1}"
}
...
resource "oci_file_storage_export" "ExportFileSystemMount" {
  ...
  path           = count.index < "9" ? "${var.export_path_base}${var.label_zs[0]}${count.index + 1}" : "${var.export_path_base}${var.label_zs[1]}${count.index + 1}"
